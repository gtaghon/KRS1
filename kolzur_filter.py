...
def kza(data, m, k):
    n = data.size
    coef = _kza_coeffs(m, k)

    # Apply the adaptive filter
    data_filtered = np.zeros_like(data)
    for i in range(k*(m-1)//2, n-k*(m-1)//2):
        window_data = data[i-k*(m-1)//2:i+k*(m-1)//2+1]
        if np.isnan(window_data).any():
            data_filtered[i] = np.nan
        else:
            window_data = window_data * coef
            data_filtered[i] = np.sum(window_data)

    return data_filtered[k*(m-1)//2:n-k*(m-1)//2]
